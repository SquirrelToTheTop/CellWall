...
void benchmarks_energy_cw(CellWallMonolayer *cwl, int mpi_rank, int mpi_size){
  ...
  std::string * ener_msg = new std::string[7] { "Spring G", "Angles G-G", "Spring P"};
	...
	int iter, i;
	int npgdim = cwl->get_total_npg()*DIM;
	double forces_xyz[npgdim];
	...
	for(iter=0; iter<10; ++iter){
		...
		start = clock();
		energy_glyco = compute_energy_gbond(cwl, &forces_xyz[0]);
		end = clock();
		cpu_time_used[0] += ((double) (end - start)) / CLOCKS_PER_SEC;

		start = clock();
		energy_glyco_glyco = compute_energy_gg_angles(cwl, &forces_xyz[0]);
		end = clock();
		...
		start = clock();
		energy_pepti = compute_energy_pbond(cwl, &forces_xyz[0]);
		end = clock();
...
void conjugate_gradient(CellWallMonolayer *cwl, int mpi_rank, int mpi_size){
	...
	cw_gbond = compute_energy_gbond(cwl, forces_xyz);
	...
	cw_pepti = compute_energy_pbond(cwl, forces_xyz);
	...
	for(k=0; k<10; ++k){
		...
		// fflush(stdout);
		for(i=0; i<npg_dim; ++i){
			d_k[i] = -forces_xyz[i] + beta_k * d_k[i];
		}
...
// 	int rand_id, i;
// 	double cw_energy_n, cw_energy_np, cw_energy_angg_n, cw_energy_anpp_n;
// 	double cw_gbond_n, cw_gbond_np, cw_pepti_n, cw_pepti_np, cw_energy_angg_np, cw_energy_anpp_np;
// 	double cw_energy_pressure_n, cw_energy_pressure_np;
...
// 	std::mt19937 rng(dev());
// 	std::uniform_real_distribution<> dis(0.0f, 1.0f);//uniform distribution between 0.0 and 0.5
// 	std::uniform_int_distribution<> rand_pg(cwl->get_number_of_pg_strand()+1,
// 																				  cwl->get_total_npg()-cwl->get_number_of_pg_strand());

// 	double save_mass_pos[DIM];
// 	double *cwl_forces = new double[cwl->get_total_npg()*DIM];

// 	char filename[32];
// 	sprintf(&filename[0], "MC_test_P%1d", mpi_rank);
// 	CellWallIOSystem *iosystem = new CellWallIOSystem(filename);
...
// 	cw_energy_angg_n = compute_energy_gg_angles(cwl);
// 	cw_energy_anpp_n = compute_energy_gp_angles(cwl);
// 	cw_energy_pressure_n = compute_energy_pressure(cwl);
// 	compute_force_pressure(cwl);
// 	cw_energy_n = cw_gbond_n + cw_pepti_n + cw_energy_angg_n + cw_energy_anpp_n;
// 	cw_energy_n += cw_energy_pressure_n;

// 	fprintf(stdout,"\n\t\t> (P%d) CW G-bond energy 0 : %f", mpi_rank, cw_gbond_n);
// 	fprintf(stdout,"\n\t\t> (P%d) CW P-spring energy 0 : %f", mpi_rank, cw_pepti_n);
// 	fprintf(stdout,"\n\t\t> (P%d) CW G-G angles 0 : %f", mpi_rank, cw_energy_angg_n);
// 	fprintf(stdout,"\n\t\t> (P%d) CW G-P energy 0 : %f", mpi_rank, cw_energy_anpp_n);
// 	fprintf(stdout,"\n\t\t> (P%d) CW pressure energy 0 : %f", mpi_rank, cw_energy_pressure_n);
...
// 		// rand_id = DIM;
// 		// rand_id = 10*DIM;
// 		// for( rand_id=0; rand_id<cwl->get_number_of_pg_strand()*DIM*2; rand_id+=DIM ){
...
// 			save_mass_pos[0] = cwl->coordinate_xyz[rand_id];
// 			save_mass_pos[1] = cwl->coordinate_xyz[rand_id]+1;
// 			save_mass_pos[2] = cwl->coordinate_xyz[rand_id+2];
			
// 			// cwl->coordinate_xyz[rand_id] +=  0.05f * cwl->forces_xyz[rand_id];
// 			// cwl->coordinate_xyz[rand_id+1] += 0.05f * cwl->forces_xyz[rand_id+1];
// 			// cwl->coordinate_xyz[rand_id+2] += 0.05f * cwl->forces_xyz[rand_id+2];
// 			cwl->coordinate_xyz[rand_id] +=  0.01f * dis(rng);
// 			cwl->coordinate_xyz[rand_id+1] += 0.01f * dis(rng);
// 			cwl->coordinate_xyz[rand_id+2] += 0.01f * dis(rng);
...
// 		cw_energy_angg_np = compute_energy_gg_angles(cwl);
// 		cw_energy_anpp_np = compute_energy_gp_angles(cwl);
// 		cw_energy_pressure_np = compute_energy_pressure(cwl);
// 		compute_force_pressure(cwl);
// 		cw_energy_np = cw_gbond_np + cw_pepti_np + cw_energy_angg_np + cw_energy_anpp_np;
// 		cw_energy_np += cw_energy_pressure_np;
...
// 		fprintf(stdout,"\n\t\t> (P%d) CW G-G angles 0 : %f", mpi_rank, cw_energy_angg_np);
// 		fprintf(stdout,"\n\t\t> (P%d) CW G-P energy 0 : %f", mpi_rank, cw_energy_anpp_np);
// 		fprintf(stdout,"\n\t\t> (P%d) CW pressure energy 0 : %f", mpi_rank, cw_energy_pressure_np);
...
// 			// if( cw_energy_np-cw_energy_n > fictive_kb*fictive_temperature )
			
// 			proba = exp(-(cw_energy_np - cw_energy_n) / (fictive_kb*fictive_temperature) );
// 			fprintf(stdout, "\n\t> dE = %f", cw_energy_np-cw_energy_n );
// 			fprintf(stdout, "\n\t> k_bT = %f", fictive_kb*fictive_temperature );
// 			fprintf(stdout, "\n\t> proba = %f", proba );
...
// 				cwl->coordinate_xyz[rand_id] = save_mass_pos[0];
// 				cwl->coordinate_xyz[rand_id+1] = save_mass_pos[1];
// 				cwl->coordinate_xyz[rand_id+2] = save_mass_pos[2];
